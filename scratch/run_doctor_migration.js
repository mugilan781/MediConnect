...
async function run() {
  const sqlFile = path.join(__dirname, '../database/migration_doctor_dashboard.sql');
  ...
  console.log(`Executing ${statements.length} statements from migration_doctor_dashboard.sql...`);
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    try {
      ...
      // If column/table already exists, we can log it instead of failing
  ...
  console.log('✅ Doctor Dashboard schema migration completed successfully!');
  process.exit(0);
}
